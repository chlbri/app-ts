 ...
 */
const reduceEvents = (_events, firstEvent, ...toChecks) => {
    const check1 = _events === machine_constants.EVENTS_FULL;
    if (check1)
        return true;
    const check2 = toChecks.every(check => {
        ...
        for (const _event of _events) {
            ...
            for (const [key, value] of entries) {
                const values = basifun.toArray.typed(value);
                if (key === firstEvent) {
...
exports.reduceEvents = reduceEvents;
