...
class Machine {
    ...
    constructor(config) {
        ...
        this.#flat = states_functions_flatMap.flatMap(config, true);
        this.#initialConfig = states_functions_initialConfig.initialConfig(this.#config);
        this.#getInitialKeys();
    }
    /**
     ...
     */
    valueToConfig = (from) => {
        return states_functions_valueToNode.valueToNode(this.#config, from);
    };
    /**
 ...
 */
const createMachine = (config, { eventsMap, pContext, context, promiseesMap }) => {
    const out = new Machine(config)
