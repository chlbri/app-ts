...
class Interpreter {
    ...
    get #childrenMachines() {
        const _machines = basifun.toArray.typed(this.#machine.config.machines);
            ...
            const callback = () => {
                const cb = () => {
                    for (const activity of activities) {
                        ...
                        const check3 = types_primitives.isDescriber(activity);
    ...
    #performTransition = transition => {
        ...
        if (check) {
            ...
            this.#performActions(...basifun.toArray.typed(diffExits));
            this.#performActions(...basifun.toArray.typed(diffEntries));
            return transition;
        }
    ...
    #performFinally = (_finally) => {
        ...
        for (const final of finals) {
            ...
            const check3 = types_primitives.isDescriber(final);
            const check4 = check2 || check3;
            if (check4) {
