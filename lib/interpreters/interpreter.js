import { reduceAction } from '../actions/functions/reduceAction.js';
import { toAction } from '../actions/functions/toAction.js';
import '../constants/errors.js';
import { DEFAULT_MAX_SELF_TRANSITIONS, DEFAULT_MAX_TIME_PROMISE, DEFAULT_MIN_ACTIVITY_TIME } from '../constants/numbers.js';
import { DEFAULT_DELIMITER } from '../constants/strings.js';
import { toDelay } from '../delays/functions/toDelay.js';
import { INIT_EVENT } from '../events/constants.js';
import { eventToType } from '../events/functions/eventToType.js';
import { possibleEvents } from '../events/functions/possibleEvents.js';
import { transformEventArg } from '../events/functions/transform.js';
...
import { getByKey, assignByKey, mergeByKey } from '../machine/functions/subcriber/contexts.js';
import { reduceEvents } from '../machine/functions/subcriber/events.js';
import { toMachine } from '../machine/functions/toMachine.js';
import { getEntries, getExits } from '../machine/machine.js';
import { toPredicate } from '../guards/functions/toPredicate.js';
import { toPromiseSrc } from '../promises/functions/src.js';
import { toArray, isDefined, withTimeout, anyPromises, switchV, asyncfy, partialCall } from '@bemedev/basifun';
import { flatMap } from '../states/functions/flatMap.js';
import { initialConfig } from '../states/functions/initialConfig.js';
import { nextSV } from '../states/functions/nextSV.js';
import { nodeToValue } from '../states/functions/nodeToValue.js';
import { resolveNode } from '../states/functions/resolveNode.js';
import { isDescriber } from '../types/primitives.js';
import { createTimeout, createInterval } from '@bemedev/interval2';
...
class Interpreter {
    ...
    _performConfig = () => {
        ...
        const configForFlat = castings.commons.unknown(this.#config);
    ...
    #resolveNode = (config) => {
        ...
        return resolveNode(events, promisees, config, options);
    };
    /**
    ...
    get select() {
        ...
        if (check)
            return undefined;
        const out = (path) => getByKey(this.#context, path);
        return out;
    }
    /**
     * @deprecated
    ...
    get _pSelect() {
        if (IS_TEST) {
            ...
            if (check)
                return undefined;
            if (pContext) {
                const out = (path) => getByKey(pContext, path);
                return out;
            }
            /* v8 ignore next 4 */
        }
    ...
    _next = async () => {
        ...
        do {
            ...
            const checkCounter = this.#selfTransitionsCounter >= DEFAULT_MAX_SELF_TRANSITIONS;
            if (checkCounter)
            ...
            const callback = () => {
                const cb = () => {
                    for (const activity of activities) {
                        ...
                        const check5 = this.#performPredicates(...toArray.typed(activity.guards));
                        if (check5) {
    ...
    #performTransition = transition => {
        ...
        const response = this.#performPredicates(...toArray(guards));
        if (response) {
            this.#performActions(...toArray(diffExits));
            this.#performActions(...toArray(actions));
            this.#performActions(...toArray(diffEntries));
            return target ?? false;
        }
        return false;
    };
    ...
    subscribe = (_subscriber, options) => {
        ...
        if (find)
            return find;
        const subcriber = createSubscriber(eventsMap, promiseesMap, _subscriber, options);
    ...
    #extractTransitions = (event) => {
        ...
        flat.forEach(([from, transitions], _, all) => {
            const canTake = all.every(([from2]) => !from2.startsWith(`${from}${DEFAULT_DELIMITER}`));
            if (canTake)
    ...
    _send = event => {
        ...
        flat2.forEach(([from, transitions]) => {
            ...
            if (cannotContinue)
                return;
            const target = this.#performTransitions(...toArray.typed(transitions));
     ...
     */
    #proposedNextSV = (target) => nextSV(this.#value, target);
    /**
    ...
    #diffNext = (target) => {
        ...
        const next = castings.commons.unknown(initialConfig(castings.commons.unknown(this.proposedNextConfig(target))));
    ...
    toActionFn = (action) => {
        ...
        return this.#returnWithWarning(toAction(events, promisees, action, actions), `Action (${reduceAction(action)}) is not defined`);
    };
    toPredicateFn = (guard) => {
        const events = this.#machine.eventsMap;
        const promisees = this.#machine.promiseesMap;
        const predicates = this.#machine.predicates;
        const { predicate, errors } = toPredicate(events, promisees, guard, predicates);
    ...
    toPromiseSrcFn = (src) => {
        ...
        return this.#returnWithWarning(toPromiseSrc(events, promisees, src, promises), `Promise (${src}) is not defined`);
    };
    toDelayFn = (delay) => {
        const events = this.#machine.eventsMap;
        const promisees = this.#machine.promiseesMap;
        const delays = this.#machine.delays;
        return this.#returnWithWarning(toDelay(events, promisees, delay, delays), `Delay (${delay}) is not defined`);
    };
    toMachine = (machine) => {
        const machines = this.#machine.machines;
        return this.#returnWithWarning(toMachine(machine, machines), `Machine (${reduceAction(machine)}) is not defined`);
    };
    ...
    subscribeMachine = (id, { initials: _initials, ...rest }) => {
        const reduced = reduceFnMap(this.#machine.eventsMap, this.#machine.promiseesMap, _initials);
        const initials = reduced(this.#cloneState);
        const child = castings.commons({
            initials,
            ...rest,
        });
    ...
    #sendTo = (to, event) => {
        ...
        if (service)
            service.send(castings.commons(event));
    };
    /**
    ...
    #reduceChild = ({ subscribers, machine, initials }, id) => {
        let service = castings.commons(this.#childrenServices.find(f => f.id === id));
        if (!service) {
        ...
        const subscriber = service.subscribe(({ event }) => {
            ...
            _subscribers.forEach(({ contexts, events }) => {
                ...
                const checkEvents = reduceEvents(castings.commons.any(events), type, type2);
                ...
                if (checkEvents) {
                    if (checkContexts) {
                        ...
                        const _contexts = castings.commons.unknown(contexts);
                        const paths = toArray.typed(_contexts);
                        paths.forEach(path => {
                            if (typeof path === 'string') {
                                const callback = () => assignByKey(this.#pContext, path, service.#context);
                                this.#schedule(callback);
                            }
                            else {
                                ...
                                entries.forEach(([pathChild, paths]) => {
                                    paths.forEach(path => {
                                        const pContext = mergeByKey(this.#pContext)(path, getByKey(service.#context, pathChild));
                                        const callback = () => this.#mergeContexts({ pContext });
                                        this.#schedule(callback);
                                    });
                                });
...
const TIME_TO_RINIT_SELF_COUNTER = DEFAULT_MIN_ACTIVITY_TIME * 2;
...
export { Interpreter, TIME_TO_RINIT_SELF_COUNTER, _interpret, interpret };
