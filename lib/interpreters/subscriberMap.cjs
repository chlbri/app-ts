...
class SubscriberMapClass {
  ...
  get #reduceFn() {
    ...
    if (check1) return types.castings.commons.any(sub);
    const map = utils_reduceFnMap.toEventsMap(
      this.#eventsMap,
      this.#promiseesMap,
    );
    ...
    return state => {
      ...
      if (check5) return types.castings.commons.any(_else(state));
      const { type } = event;
      ...
      return types.castings.commons.any(_else(state));
    };
  }
  get #cannotPerform() {
...
exports.createSubscriberMap = createSubscriberMap;
//# sourceMappingURL=subscriberMap.cjs.map
