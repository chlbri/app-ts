...
const isValue = (path, ...values) => {
    ...
    return ({ pContext, context, event }) => {
        ...
        if (start('context.')) {
            ...
            return values.some(value => machine_functions_subcriber_contexts.getByKey(context, key) === value);
        }
        if (start('pContext.')) {
            const key = path.replace('pContext.', '');
            return values.some(value => machine_functions_subcriber_contexts.getByKey(pContext, key) === value);
        }
        const key = path.replace('events.', '');
        const toValidate = machine_functions_subcriber_contexts.getByKey(event, key);
