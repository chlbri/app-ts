...
function resolve(basePath, relativePath) {
    if (relativePath === constants_strings.DEFAULT_DELIMITER)
        return constants_strings.DEFAULT_DELIMITER;
    ...
    const endsWithSlash = relativePath.endsWith(constants_strings.DEFAULT_DELIMITER);
    ...
    if (nonParentSegments.length === 0 && parentLevels > 0) {
        ...
        if (resultSegments.length >= 0) {
            ...
            if (targetLevel >= 0 && targetLevel < originalBaseSegments.length) {
                ...
                return (constants_strings.DEFAULT_DELIMITER +
                    resultSegments.join(constants_strings.DEFAULT_DELIMITER) +
                    (resultSegments.length > 0 ? constants_strings.DEFAULT_DELIMITER : '') +
                    targetDir);
            }
        }
        return constants_strings.DEFAULT_DELIMITER + resultSegments.join(constants_strings.DEFAULT_DELIMITER);
    }
    ...
    if (nonParentSegments.length > 0 &&
        ...
        if (segmentIndex !== -1) {
            ...
            if (alternativeResult.length > resultSegments.length ||
                (alternativeResult.length === resultSegments.length &&
                    alternativeResult[alternativeResult.length - 1] === finalSegment)) {
                return (constants_strings.DEFAULT_DELIMITER + alternativeResult.join(constants_strings.DEFAULT_DELIMITER));
            }
        }
    }
