...
const reduceFnMap = (events, promisees, fn) => {
    ...
    return ({ event, context, pContext, status, value, tags }) => {
        ...
        if (check5)
            return castings.commons.any(_else({ event, context, pContext, status, value, tags }));
...
const reduceFnMapReduced = (events, promisees, fn) => {
    const check1 = isFunction(fn);
    if (check1)
        return castings.commons.any(fn);
    const map = toEventsMap(events, promisees);
    const keys = Object.keys(map);
    return ({ context, event, status, value, tags }) => {
        ...
        const _else = fn.else ?? nothing;
        if (check5)
            return castings.commons.any(_else({ context, event, status, value, tags }));
        const { payload, type } = event;
        for (const key of keys) {
            const check2 = type === key;
            const func = castings.commons.any(fn)[key];
...
export { reduceFnMap, reduceFnMapReduced, toEventsMap };
//# sourceMappingURL=reduceFnMap.js.map
